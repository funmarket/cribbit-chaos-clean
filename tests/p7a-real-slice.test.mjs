...
dbTest('P7A real API creates, joins, starts, draws, persists and syncs canonical users', async () => {
  await withIsolatedP6Database(connectionString, async (pool) => {
    const handler = createNodeApiHandler({ databaseUrl: connectionString, checkConnection: async () => {}, pool, sessionSecret: 'db-test-secret' });
    await withServer(handler, async (baseUrl) => {
      const auth = async (displayName) => {
      ...
      const ada = await auth('Ada');
      const ben = await auth('Ben');

      const created = await json(await fetch(`${baseUrl}/api/sessions`, {
        method: 'POST', headers: { 'content-type': 'application/json', cookie: ada.cookie }, body: JSON.stringify({ displayName: 'Ada' })
      }));
      assert.equal(created.projection.status, 'waiting');
      assert.equal(created.player.playerId, 'p1');

      const outsider = await fetch(`${baseUrl}/api/sessions/${created.player.sessionId}/projection`);
      assert.equal(outsider.status, 401);

      const joined = await json(await fetch(`${baseUrl}/api/sessions/${created.player.sessionId}/join`, {
        method: 'POST', headers: { 'content-type': 'application/json', cookie: ben.cookie }, body: JSON.stringify({ displayName: 'Ben' })
      }));
      ...
      const started = await json(await fetch(`${baseUrl}/api/sessions/${created.player.sessionId}/start`, {
        method: 'POST', headers: { cookie: ada.cookie }
      }));
      assert.equal(started.projection.status, 'active');
      assert.equal(started.projection.currentPlayer.hand.length, 7);

      const bStarted = await json(await fetch(`${baseUrl}/api/sessions/${created.player.sessionId}/projection`, {
      ...
      const bAfter = await json(await fetch(`${baseUrl}/api/sessions/${created.player.sessionId}/projection`, {
        headers: { cookie: ben.cookie }
      }));
...
test('P7A api-client consumes typed session projections and commands through its fetch boundary', async () => {
  ...
  const fakeFetch = async (url, init = {}) => {
    ...
    return new Response(JSON.stringify({
      player: { sessionId: 'abc123', playerId: 'p1', displayName: 'Ada' },
  ...
  assert.equal(result.player.playerId, 'p1');
...
test('P7A frontend bootstraps pass VITE_API_URL while preserving an empty-env fallback', async () => {
  ...
  assert.match(clientSource, /createCribbitApiClient\(\{ baseUrl: normalizeApiBaseUrl\(options\.apiBaseUrl\), getAuthHeaders: platform\.getAuthHeaders \}\)/);
