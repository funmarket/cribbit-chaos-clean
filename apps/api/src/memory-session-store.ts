...
import type { AcceptedCommandReceipt, GameCommandPayload, GameViewProjection } from '@cribbit/contracts';
import type { CommandSessionTransaction, CommandTransactionPort, SessionMembership } from './ports.ts';
...
const cloneState = (state: CanonicalGameState): CanonicalGameState => structuredClone(state);

export function resetMemorySessions(): void { sessions.clear(); }
...
export function createMemorySessionStore() {
  return {
    async createSession(input: { readonly sessionId: string; readonly principalId: string; readonly playerId: string; readonly displayName: string }): Promise<CreatedSessionRecord> {
      ...
      sessions.set(input.sessionId, { state, members: [{ principalId: input.principalId, playerId: input.playerId, displayName: input.displayName }], receipts: new Map() });
    ...
    async loadForPrincipal(sessionId: string, principalId: string): Promise<LoadedPlayerSession | null> {
      ...
      return { playerId: member.playerId, state: cloneState(session.state), projection: projectGameView(session.state, member.playerId) };
    },

    async startSession(input: { readonly sessionId: string; readonly principalId: string; readonly shuffledDeck: readonly string[] }): Promise<SessionStoreResult<{ projection: GameViewProjection }>> {
      const session = sessions.get(input.sessionId);
      if (!session) return { status: 'rejected', reason: 'SESSION_NOT_FOUND' };
      ...
      if (session.state.lifecycle?.hostPlayerId !== member.playerId) return { status: 'rejected', reason: 'ONLY_HOST_CAN_START' };
  ...
  return {
    async withSession<T>(sessionId: string, run: (transaction: CommandSessionTransaction) => Promise<T>): Promise<T | null> {
      ...
      const transaction: CommandSessionTransaction = {
        ...
        async loadCanonicalState() { return cloneState(session.state); },
        async findAcceptedReceipt(commandId: string) { return session.receipts.get(commandId) ?? null; },
        async commitAccepted(input) {
          if (session.state.revision !== input.expectedRevision) throw new Error('SESSION_REVISION_CONFLICT');
