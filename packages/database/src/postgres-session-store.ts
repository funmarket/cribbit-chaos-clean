...
async function updateState(client: PoolClient, state: CanonicalGameState, previousRevision: number): Promise<void> {
  ...
  await client.query(
    ...
    [
      ...
      JSON.stringify({ kind: 'SESSION_REVISION_COMMITTED', sessionId: state.gameId, revision: state.revision })
    ]
  );
}
...
export function createPostgresSessionStore(pool: Pool) {
  return {
    ...
    async joinSession(input: {
      ...
      readonly playerId: string;
      readonly displayName: string;
    }): Promise<SessionStoreResult<LoadedPlayerSession>> {
      const client = await pool.connect();
      try {
        await client.query('begin');
    ...
    async startSession(input: {
      ...
      try {
        ...
        if (membership.rowCount === 0) { await client.query('rollback'); return { status: 'rejected', reason: 'NOT_SESSION_MEMBER' }; }
        const playerId = membership.rows[0].player_id;
